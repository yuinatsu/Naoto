	...
	if (timeflag_)
	{
		...
		if (pageflag_ == false)
		{
				...
				(pos_.x <= sctPosE + lpSize.titleSect3_.x) && (pos_.y <= sctPosF + lpSize.titleSect3_.y))
			{
				...
				if (lpMouse.GetPresCnt(Mouse::LMB) == 1)
				{
					TRACE("GameSceneからTitleSceneに移行\n");
					//ChangeVolumeSoundMem(255 * lpSceneCtl.CVS / 100, atfSE_);//音量設定
					//PlaySoundMem(atfSE_, DX_PLAYTYPE_NORMAL, true);
					Alltime += aftime;
					PlaySoundMem(click_SE_, DX_PLAYTYPE_BACK, true);
...
bool GameScene::Init(void)
{
	...
	/*VecInt& ImageMng::GetID(std::string f_name, std::string key, Vector2 divSize, Vector2 divCnt)
	LoadDivGraph(f_name.c_str(), (divCnt.x * divCnt.y), divCnt.x, divCnt.y, divSize.x, divSize.y, &imgMap_[key][0], true);*/
	
	//LoadDivGraph("Resource/image/blood1.png", divCnt.x * divCnt.y, divCnt.x, divCnt.y, divsize.x, divsize.y, MENBOimage_, true);
	//effectImage_[0] = LoadDivGraph("Resource/image/blood1.png", divCnt.x * divCnt.y, divCnt.x, divCnt.y, divsize.x, divsize.y, MENBOimage_, true);
	...
	//WeaponImage_[0] = LoadGraph(GH_FLAME_COTTON);
	//WeaponImage_[1] = LoadGraph(GH_FLAME_PICK);
	//WeaponImage_[2] = LoadGraph(GH_FLAME_WRIST_CUTTER_G);
	//WeaponImage_[3] = LoadGraph(GH_FLAME_DEEPKISS);
	//WeaponImage_[4] = LoadGraph(GH_FLAME_OMORI);
	//WeaponImage_[5] = LoadGraph(GH_ICON_NODON);

	//IconImage_[0] = LoadGraph(GH_ICON_COTTON);
	//IconImage_[1] = LoadGraph(GH_ICON_PICK);
	//IconImage_[2] = LoadGraph(GH_ICON_WRIST_CUTTER_G);
	//IconImage_[3] = LoadGraph(GH_ICON_DEEPKISS);
	//IconImage_[4] = LoadGraph(GH_ICON_OMORI);
	//IconImage_[5] = LoadGraph(GH_ICON_NODON);
	...
	mimiSpd_ = 3;
	//lpMapCtl.MapLoad("MapData.sav", *this);
...
bool GameScene::Release(void)
{
	...
	/*for (int i = 0; i < 5; i++)
	{
		DeleteGraph(lpSceneCtl.mimiImage_[i]);
	}*/
	for (int num = 0; num < static_cast<int>(WEAPON::MAX); num++)
	{
...
void GameScene::DrawEffect(int num)
{
	...
	if (effectflag)
	{
		...
		if (num == 3)
		{
			...
			//if (effectCnt_ / Divide % nine > 7)//落ちるエフェクト
			//{
			//	DrawGraph(posefct.x, ++posefct.y, lpImageMng.GetID("kiss")[nine], true);
			//}
		...
		//DrawGraph(posefct.x, posefct.y, effectImage_ [[0][effectCnt_ / 10]], true);
		//DrawBox(10, 10, 20, 20, 0xff0000, false);	//試し
	}
}
...
void GameScene::DrawPause(void)
{
	if (timeflag_)
	{
		...
		/*DrawString(720 / 2, 45, "タイトルに戻る", 0xff0000);
		DrawString(720 / 2, 60, "pushSpace", 0xff0000);
		DrawString(pos_.x, pos_.y, "右クリックでタイマー開始", 0xff0000);*/
