...
void MapCtl::Update(void)
{
	for (int y = 0; y < MAP_CHIP_COUNT_Y; y++)														// マップの縦のブロック数
	{
		...
		{
			if ((mapData[y][x] > 0) && (mapData[y][x] < BLOCK_MAX_SIZE_CNT))						// mapData[y][x]が０以上BLOCK_MAX_SIZE_CNT未満だった場合加算
			{
				mapData[y][x]++;
			}
		}
...
void MapCtl::Draw(void)
{
	for (int y = 0; y < MAP_CHIP_COUNT_Y; y++)														// マップの縦のブロック数
	{
		for (int x = 0; x < MAP_CHIP_COUNT_X; x++)													// マップの横のブロック数
		{
			...
			{
				//int mapID = mapData[y][x] / BLOCK_CNT_INV;					// (mapData[y][x] / BLOCK_CNT_INV) % BLOCK_PTN_MAX　の別解
				//mapID = (mapID < BLOCK_PTN_MAX ? mapID : 0);					// 参照演算子
...
bool MapCtl::isOnBlock(Vector2 pos)
{
	...
	// return (GetBlock(pos) >= BLOCK_MAX_SIZE_CNT);							// 一行で書くならこっち			比較演算子でも演算子でも（）内ですでに計算されているので、if文を省略できる		"!"を先頭に書けばreturn内容を逆にできる
