...
bool CharImage::Init(CHAR_ID charID)
{
	...
	if ((LoadDivGraph(("Resource/image/" + charNameList[static_cast<int>(charID)] + "_chip.png").c_str(), ANIM_IMAGE_MAX * static_cast<int>(DIR::MAX), ANIM_IMAGE_MAX, static_cast<int>(DIR::MAX), CHAR_SIZE_X, CHAR_SIZE_Y, &chipImage[0][0])) == -1)						// キャラチップ			// .c_str()		stringをcharに変更
	{
		TRACE("読み込みエラー1\n");
		rtnFlag = false;
	}
