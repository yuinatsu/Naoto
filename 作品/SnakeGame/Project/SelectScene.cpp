...
SCN_ID SelectScene::Update(char* keyData, char* keyDataOld)
{
	...
	for (int no = 0; no < PLAYER_MAX; no++)												// プレイヤー数分
	{
		...
		// SelectCharの中身
		//if (player[no].keyNow_[static_cast<int>(DIR::LEFT)] && !(player[no].keyOld_[static_cast<int>(DIR::LEFT)]))			// 左キーを押してキャラセレクト
		//{
		//	if (player[no].charID_ != CHAR_ID::REIMU)
		//	{
		//		player[no].charID_ = static_cast<CHAR_ID>(static_cast<int>(player[no].charID_) - 1);							// REIMU,MARISA,CIRNO,SAKUYA,YOUMUの順でキャラセレクト
		//	}
		//	else
		//	{
		//		player[no].charID_ = CHAR_ID::YOUMU;																				// REIMUに達していた場合YOUMUに移動
		//	}
		//}
		//if (player[no].keyNow_[static_cast<int>(DIR::RIGHT)] && !(player[no].keyOld_[static_cast<int>(DIR::RIGHT)]))			// 右キーを押してキャラセレクト
		//{
		//	if (player[no].charID_ != CHAR_ID::YOUMU)
		//	{
		//		player[no].charID_ = static_cast<CHAR_ID>(static_cast<int>(player[no].charID_) + 1);							// YOUMU,SAKUYA,CIRNO,MARISA,REIMUの順でキャラセレクト
		//	}
		//	else
		//	{
		//		player[no].charID_ = CHAR_ID::REIMU;																				// YOUMUに達していた場合REIMUに移動
		//	}
		//}

		// DesideCharの中身
		//if (player[no].keyNow_[static_cast<int>(DIR::UP)] && !(player[no].keyOld_[static_cast<int>(DIR::UP)]))			// 上キーを押してキャラセレクト決定
		//{
		//	player[no].state_ = PL_ST::ALIVE;																				// 決定
		//}
		//if (player[no].keyNow_[static_cast<int>(DIR::DOWN)] && !(player[no].keyOld_[static_cast<int>(DIR::DOWN)]))		// 下キーを押してキャラセレクトキャンセル
		//{
		//	player[no].state_ = PL_ST::DEAD;																				// キャンセル
		//}
	}
	...
	// 2人対戦以外でする場合はこちらのほうがよい
	//for (int no = 0; no < PLAYER_MAX; no++)
	//{
	//	if (player[no].state_ == PL_ST::ALIVE)
	//	{
	//		return SCN_ID::SELECT;						// NONが１人でもいたらSELECTに戻る
	//	}
	//}
	//return SCN_ID::GAME;								// NONが1人もいなかったらGAMEに移動
}
...
bool SelectScene::Init(void)
{
	...
	GetScreenState(&screenSize_.x, &screenSize_.y, nullptr);				// スクリーンサイズの値の取得
