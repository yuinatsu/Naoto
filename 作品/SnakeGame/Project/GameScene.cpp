...
SCN_ID GameScene::Update(char* keyData, char* keyDataOld)
{
	...
	if (callCnt_ >= CALL_COUNT_MAX * 2)
	{
		callFlag_ = false;
	}
	// 上のfor文の中身
	//------------------
	/*if (callCnt1_ < CALL_COUNT_MAX)
	{
		callCnt1_++;
		return SCN_ID::GAME;
	}
	if (callCnt1_ < CALL_COUNT_MAX * 2)
	{
		callCnt1_++;
		return SCN_ID::GAME;
	}*/
...
void GameScene::StartCallDraw(void)
{
	...
	/*if (!(callCnt1_ >= CALL_COUNT_MAX))
	{
		if (callCnt1_ >= 100)
		{
			DrawGraph((screenSize_.x - CALL_IMAGE_SIZE_X) / 2, (screenSize_.y - CALL_IMAGE_SIZE_Y) / 2, callImage[0], true);
		}
	}
	if (!(callCnt1_ >= CALL_COUNT_MAX * 2))
	{
		if ((callCnt1_ >= 100 * (2 + 1)))
		{
			DrawGraph((screenSize_.x - CALL_IMAGE_SIZE_X) / 2, (screenSize_.y - CALL_IMAGE_SIZE_Y) / 2, callImage[1], true);
		}
	}*/
}
