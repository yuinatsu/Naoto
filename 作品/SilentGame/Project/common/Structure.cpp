...
Fan::Fan(const Math::Vector2& p, const Math::Vector2& inv, float angle, ColList col1, ColList col2) :
...
void Fan::Draw( const Math::Vector2& offsetpos)
{
	...
	Math::Vector2 p = center+ offsetpos + v1;
	DrawLine(center.x + offsetpos.x, center.y + offsetpos.y, p.x, p.y, color);
	p = center + v2 + offsetpos;
	DrawLine(center.x + offsetpos.x, center.y + offsetpos.y, p.x, p.y, color);
}
...
void Squar::Draw(const Math::Vector2& offsetpos)
{
	...
	DrawBox(sPos.x + offsetpos.x, sPos.y + offsetpos.y, ePos.x + offsetpos.x, ePos.y + offsetpos.y, color, true);
}
