...
{
	/// ゲームによって変わる部分------------------
	const int seNum = 5;			// SEの数
	const int bgmNum = 1;			// BGMの数
	...
	void DoSound(ESOUNDNAME_SE name, int volume)
	{
		...
		{
			// 音量を消して再生しなおす
			ChangeVolumeSoundMem(volume, se_sound[static_cast<int>(name)]);
			PlaySoundMem(se_sound[static_cast<int>(name)], DX_PLAYTYPE_BACK);
		}
	} /// void DoSound(ESOUNDNAME_SE name, int volume)
	...
	void SEEnd(ESOUNDNAME_SE name)
	{
		if (se_playFlag[static_cast<int>(name)] == true)
		{
			ChangeVolumeSoundMem(0, se_sound[static_cast<int>(name)]);
		}
	}
